...
!{ GEOSR GATE : jgcho 2015.06.19
        ...
        IF (MOD(N,IATS(LG)).eq.0) THEN
          ...
          HUPG(LG)=AHUP(LG)/float(IATS(LG))
          AHUP(LG)=0.
        ELSE !IF (MOD(N,IATS).eq.0) THEN
          IF (N.EQ.1) THEN
            HUPG(LG)=HUP0
            AHUP(LG)=HUP0
          ELSE
...
! { GEOSR FIXED WEIR TS remove : jgcho 2014.10.14
  ...
  200     CONTINUE
          M2=M1+1
          IF (TIMEDAY.GT.GCSER(M2,NQCTLQ(NCTL))) THEN
            M1=M2
            GOTO 200
          ELSE
...
!{ GEOSR GATE : jgcho 2016.07.14 가동보 가동시 상류수위가 가동보 상단보다 높은경우에도 월류식 적용
                  if (HUPG(LG).gt.SILLHH(LG)+GUPHT) then ! 상류수위가 가동보상단보다 높을 경우
...
!     &        					*SQRT(2.*9.806*(h11-h22))
     ...
     &                       *SQRT(2.*9.806*DELHG(LG))
                          ENDIF  ! IF ((GUPHT+SILL(LG)).GT.HDWG(LG)) THEN
!
!{ GEOSR GATE : jgcho 2016.07.14 회전식 가동보 가동시 상류수위가 가동보 상단보다 높은경우에도 월류식 적용
                        if (HUPG(LG).gt.SILLHH(LG)+GUPHT*(-1.)) then ! 상류수위가 가동보상단보다 높을 경우
...
!     &      .AND. HUPG(LG).GT.SEL1T )THEN   ! UPSTREAM -> DOWNSTREAM 
     &      .AND. HUPG(LG).GT.SILL(LG) )THEN   ! UPSTREAM -> DOWNSTREAM ! 2016.06.01 jgcho
            IF (NGCHECK(LG).EQ.0) THEN                        ! GATE OPEN CHECK
!            IF (NGCHECK(LG).EQ.0
!     &          .or. MOD(N,IATS(LG)).eq.0 .or. N.eq.1) THEN                        ! GEOSR GATE : jgcho 2015.06.19
              NGCOUNT(LG)=1                                   ! TIME STEP COUNT AFTER GATE OPEN
              NGCHECK(LG)=1                                   ! GATE OPEN CHECK
            ELSE
              NGCOUNT(LG)=NGCOUNT(LG)+1                       ! ADD TIME STEP 
...
!            IF (HDWG(LG) .LT. SEL1T ) THEN   ! (1) IF (DEPDWG(LG) .LT. SILL(LG))
            IF (HDWG(LG) .LT. SILL(LG) ) THEN   ! (1) IF (DEPDWG(LG) .LT. SILL(LG)) ! 2016.06.01 jgcho
              IGCHECK(LG)=71
              tcdiff=CGH2(LG)-CGH1(LG)
!              h11=HUPG(LG)-SEL1T
              h11=HUPG(LG)-SILL(LG) ! 2016.06.01 jgcho
              wctm1=(CGH2(LG)-h11)/tcdiff
              wctm2=(h11-CGH1(LG))/tcdiff
              CG10=wctm1*CG1(LG) + wctm2*CG2(LG)
...
!     					IF((HUPG(LG)-SEL1T).LT.CGH1(LG)) THEN
     					IF((HUPG(LG)-SILL(LG)).LT.CGH1(LG)) THEN ! 2016.06.01 jgcho
     						CG10=CG1(LG)
!                      ELSEIF((HUPG(LG)-SEL1T).GT.CGH2(LG)) THEN
     					ELSEIF((HUPG(LG)-SILL(LG)).GT.CGH2(LG)) THEN ! 2016.06.01 jgcho
     						CG10=CG2(LG)
                      ENDIF		
! { 2016.06.01 jgcho SEL1T -> SILL(LG)
...
!     &                 *SQRT(2.*9.806*(HUPG(LG)-SEL1T))     ! 2014.10.14 jgcho SILL(LG) -> SEL1T
              DUMPG(LG)=CG10*GATEWI(LG)*(HUPG(LG)-SILL(LG))
...
!     &                  *((HUPG(LG)-SEL1T)-(HDWG(LG)-SEL1T))) ! 2014.10.14 jgcho SILL(LG) -> SEL1T
              DUMPG(LG)=CG10*2.6*GATEWI(LG)*(HDWG(LG)-SILL(LG))
     &           				*SQRT(2.*9.806
...
!		  GQSUMT=GAAA*DELHG(LG)+GBBB
		  GQSUMT=GAAA*HUPG(LG)+GBBB ! jgcho 2016.06.15
            if (GQSUMT.gt.GQPHI) GQSUMT=GQPHI ! jgcho 2016.07.20
!      WRITE(*,*) GLOLEV,GHILEV,GQPLO,GQPHI,HUPG(LG),GAAA,GBBB,GQSUMT
!      STOP
! { jgcho 2016.06.15
...
! } jgcho 2016.06.15
          ...
          IF (HUPG(LG).GT.HDWG(LG)
     &      .AND. HUPG(LG).GT.SEL1T )THEN   ! jgcho 2016.06.15
!     &      .AND. HUPG(LG).GT.SILL(LG) )THEN   ! UPSTREAM -> DOWNSTREAM 
