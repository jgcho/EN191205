...
C		ALLOCATE(IZONE(NSMZM))
C		ALLOCATE(XBFCOD(NSMZM))
C		ALLOCATE(XBFNH4(NSMZM))
C		ALLOCATE(XBFNO3(NSMZM))
C		ALLOCATE(XBFO2(NSMZM))
C		ALLOCATE(XBFPO4D(NSMZM))
C		ALLOCATE(XBFSAD(NSMZM))
...
!{ GEOSR BENTHIC FLUX FOR ANOXIC ENV : YSSONG 2015.09.08      
      IF(IBENZ.LT.NSMZ)THEN
        WRITE(*,*) 'ERROR : IBENZ(WQBENFLX.INP) SHOLUD BE LESS THAN NSMZ
     & WQ3DWC.INP'
        PAUSE
      ENDIF
      IF(IWQBENOX.NE.0.AND.IBENZ.LT.3)THEN
        WRITE(*,*) 'ERROR : IBENZ(WQBENFLX.INP) MUST BE 3'
        PAUSE
      ENDIF
