...
C GC2**  READ GATE INFORMATION 1
      ...
      CALL SEEK('GC2')  
      DO L=1,NQCTL
        READ(1,*,ERR=1000) NGATEM(L),NGATEC(L),NGTYP(L) !NGATE(L),NGATEC(L),NGTYP(L)
        WRITE(7,1002)NCARD
        WRITE(7,*) NGATEM(L),NGATEC(L),NGTYP(L) !NGATE(L),NGATEC(L),NGTYP(L)
      ENDDO
C
C GC3** READ GATE INFORMATION 2
     ...
     &                    ,GOTIME(L),GCTIME(L),IATS(L) ! GEOSR GATE : jgcho 2015.06.19
        WRITE(7,1002)NCARD
        WRITE(7,*) SILL(L),SILLHH(L),GATEHI(L),GATEHO(L)
     &            ,GATEWI(L),GATEWO(L),MAXQ(L),GOTIME(L),GCTIME(L)
     &            ,IATS(L) ! GEOSR GATE : jgcho 2015.06.19
      ENDDO
C
...
C GC5** READ GATE CONTROL
     ...
     &                    ,DUM,DUM,TIDCHK(L) ! SEL1, 2(L) -> DUM
        WRITE(7,1002)NCARD
        WRITE(7,*) DELHINOUT(L),DELHSEL1(L),DUM,DUM ! SEL1, 2(L) -> DUM
     &            ,TIDCHK(L),L
      ENDDO
C
